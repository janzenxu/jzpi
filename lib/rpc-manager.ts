...
export class AgentSessionWrapper {
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          shutdownHandler: () => this.emit({
            ...
            method: "notify",
            notifyType: "warning",
            message: "Extension requested shutdown, but shutdown is not supported in JZPI.",
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      getAllThemes: () => [],
      getTheme: () => undefined,
      setTheme: () => ({ success: false, error: "Theme switching is not supported in JZPI extension UI yet" }),
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    if (toolNames !== undefined) {
      ...
      // tool registry — so they were unavailable in JZPI sessions even though the
    ...
    // extensions stay usable in JZPI just like in the `pi` CLI.
